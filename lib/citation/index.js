...
;
const { writeInlineWarning } = require('../helper');
...
module.exports = function citation_plugin(md, options) {
    ...
    md.renderer.rules["id_link"] = function (tokens, idx, _options, env, slf) {
        ...
        if(linkName)
            return ['<a href="#', tokens[idx].info, '">', linkName.refName, linkName.index.join("-"), '</a>'].join("")
        else
            return writeInlineWarning("Invalid link")
    }
