...
module.exports.getContainerRenderer = function (rules) {
    ...
    let validate = function (params) {
        let keyword = params.split(" ")[0]
        return keys.indexOf(keyword) > -1
    }
    let render = function (tokens, idx, _options, env, slf) {
        let keyword = tokens[idx].tag
        if (rules[keyword]) {
            return rules[keyword](tokens, idx, _options, env, slf)
        } else {
            if (keyword[0].charCodeAt(0) < 90 /* if Uppercase*/) {
                let msg = ["Unknow container name \"", keyword, "\"\n"].join("")
                return warningRender(msg, tokens[idx].nesting)
            } else {
                return simpleRender(keyword, tokens[idx].info, tokens[idx].nesting)
            }
        }
    }
